...
namespace UbntTools {
	...
	public class CompilanceTestDialog : Dialog {
		...
		public CompilanceTestDialog () {
			...
			LbStatus = new Label { BackgroundColor = Xwt.Drawing.Colors.White, WidthRequest = 350, Text = "Depuracion" };
			table.Add (LbStatus, 2, 0, rowspan: 3);

			BtnConnect = new Button ("_Connect");
			table.Add (BtnConnect, 0, 3);
			...
			BtnClose = new DialogButton ("Salir", Command.Close);
		...
		void BackgroundWorker_DoWork (object sender, DoWorkEventArgs e)
		{
			...
			using (var sshclient = new SshClient(_ip, _user, _pwd)) {
				try {
					...
					{
						//						cmd.Execute();
						//						version = cmd.Result.TrimEnd();
						//LbStatus.Text += "Version del Producto:  " + cmd.Result.TrimEnd() + "\r\n";
					}


					//LbStatus.Text += "Version Válida\r\n";
					...
					//LbStatus.Text += "Contry Codes Creados\r\n";
					//LbStatus.Text += "Proceso de configuracion ha terminado corectamente.\r\n";
